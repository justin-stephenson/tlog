...
from test_tlog import check_recording
from test_tlog import mklogfile
from test_tlog import ssh_pexpect
from config import *
...
class TestTlogRecSession:
    ...
    def test_conf_file_var(self, session_env_config_setup):
        ...
        sessionclass_tmp = TlogRecSessionConfig(writer="file", file_writer_path=logfile,
                                                notice=input_notice)
    ...
    def test_conf_text_var(self, session_env_config_setup):
        ...
        tmp_notice_msg = "temp"
        tmp_conf_file = TMP_TLOG_REC_SESSION_CONF
        text_env_conf_file = TEXT_ENV_TLOG_REC_SESSION_CONF
        ...
        shell = ssh_pexpect(self.user1 , 'Secret123', 'localhost')
        shell.sendline(f'export TLOG_REC_SESSION_CONF_FILE={tmp_conf_file}')
        shell.sendline('export TLOG_REC_SESSION_CONF_TEXT=\'{"notice":"TEXT Notice"}\'')
        shell.sendline(TLOG_REC_SESSION_PROG)
    ...
    def test_conf_shell_var(self, session_env_config_setup):
        ...
        sessionclass = TlogRecSessionConfig(writer="journal", shell='/bin/bash')
        sessionclass.generate_config(SYSTEM_TLOG_REC_SESSION_CONF)

        shell = ssh_pexpect(self.user1 , 'Secret123', 'localhost')
        shell.sendline('export TLOG_REC_SESSION_CONF_TEXT=\'{"shell":"/bin/zsh"}\'')
...
class TestTlogRec:
    ...
    def test_conf_text_var(self, rec_env_config_setup):
        ...
        tmp_notice_msg = "temp"
        tmp_conf_file = TMP_TLOG_REC_CONF
        text_env_conf_file = TEXT_ENV_TLOG_REC_CONF
